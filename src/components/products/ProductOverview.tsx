...
export function ProductOverview({
  ...
  return (
    <>
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center max-w-7xl mx-auto">
            ...
            <div className="space-y-6 lg:space-y-8">
              <div>
                ...
                {hook && (
                  <p className="text-sm lg:text-lg text-gray-800 font-medium italic mb-2 lg:mb-8">
                    {hook}
                  </p>
                )}
                <p className="text-xs lg:text-base text-gray-900 leading-relaxed">
                  {description}
                </p>
              </div>
            </div>
            ...
            <div className="relative">
              ...
              >
                ...
                <Button
                  variant="outline"
                  size="icon"
                  className="absolute rounded-full left-3 lg:left-5 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white shadow-md w-9 h-9 lg:w-12 lg:h-12 transition-all duration-300 ease-in-out"
                  onClick={(e) => {
                    e.stopPropagation()
                    prevImage()
                  }}
                ...
                <Button
                  variant="outline"
                  size="icon"
                  className="absolute rounded-full right-3 lg:right-5 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white shadow-md w-9 h-9 lg:w-12 lg:h-12 transition-all duration-300 ease-in-out"
                  onClick={(e) => {
                    e.stopPropagation()
                    nextImage()
                  }}
      ...
      {isModalOpen && (
        <div
          className="fixed inset-0 z-50 cursor-grab bg-black/85  flex flex-col items-center justify-center p-4"
          onClick={closeModal}
        >
          <Button
            variant="outline"
